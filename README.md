...
![robusta-magerun Logo](.github/doc/magerun-logo.png)
...
| **Latest Release**     | ![Tests](https://github.com/netz98/robusta-magerun2/actions/workflows/magento_platform_tests.yml/badge.svg?branch=master) ![https://packagist.org/packages/robusta/magerun2](https://poser.pugx.org/robusta/magerun2/v/stable.png) ![Maintenance Badge](https://img.shields.io/maintenance/yes/2022.svg) |
|------------------------|----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| **Development Branch** | ![Tests](https://github.com/netz98/robusta-magerun2/actions/workflows/magento_platform_tests.yml/badge.svg?branch=develop)                                                                                                                                                                       |
...
(<https://github.com/netz98/robusta-magerun>).
...
- 2.2.x Open Source/Commerce (last compatible robusta-magerun2 version is v3.2.0)
...
Download the latest stable Robusta-Magerun phar-file from the [file-server](https://files.magerun.net/):

```sh
wget https://files.magerun.net/robusta-magerun2.phar
```

or if you prefer to use Curl:

```sh
curl -O https://files.magerun.net/robusta-magerun2.phar
```
...
```sh
shasum -a256 robusta-magerun2.phar
```

It is also possible to verify automatically:

```sh
curl -sS -O https://files.magerun.net/robusta-magerun2-latest.phar
curl -sS -o robusta-magerun2-latest.phar.sha256 https://files.magerun.net/sha256.php?file=robusta-magerun2-latest.phar
shasum -a 256 -c robusta-magerun2-latest.phar.sha256
```
...
```sh
sudo cp ./robusta-magerun2.phar /usr/local/bin/
```

### Install the phar via Composer

We offer a special dist package to install the phar file via Composer.
See <https://packagist.org/packages/robusta/magerun2-dist> for more details.
...
```sh
./robusta-magerun2.phar self-update [--dry-run]
```
...
To install the completion files, copy `robusta-magerun2.phar.bash` to your
bash compatdir folder for autocompletion.

On my Ubuntu system this can be done with the following command:

```sh
sudo cp res/autocompletion/bash/robusta-magerun2.phar.bash /etc/bash_completion.d/
```
...
```sh
php robusta-magerun2.phar list
```
...
| `--skip-root-check`                  | Do not check if robusta-magerun2 runs as root.  |
...
```sh
robusta-magerun2.phar open-browser [store]
```
...
```sh
robusta-magerun2.phar  customer:info [email] [website]
```
...
```sh
robusta-magerun2.phar  customer:create [email] [password] [firstname] [lastname] [website]
```

Example:

```sh
robusta-magerun2.phar customer:create foo@example.com password123 John Doe base
```

You can add additional any number of custom fields, example:

```sh
robusta-magerun2.phar customer:create foo@example.com passworD123 John Doe base taxvat DE12345678 prefix Mrs.
```
...
```sh
robusta-magerun2.phar  customer:list [--format[="..."]] [search]
```

### Change customer password

```sh
robusta-magerun2.phar customer:change-password [email] [password] [website]
```
...
```sh
robusta-magerun2.phar customer:token:create <email>
```

### Delete customer

```sh
robusta-magerun2.phar customer:delete [-f|--force] [-a|--all] [-r|--range] [--fuzzy] [--id=ID] [--website=ID] [--email=EMAIL] [--firstname=STRING] [--lastname=STRING]
```

Examples:
```sh
robusta-magerun2.phar customer:delete --id 1                     # Will delete customer with Id 1
robusta-magerun2.phar customer:delete --fuzzy --email=test       # Will delete all customers with email like "%test%"
robusta-magerun2.phar customer:delete --all                      # Will delete all customers
robusta-magerun2.phar customer:delete --range                    # Will prompt for start and end Ids for batch deletion
```
...
```sh
robusta-magerun2.phar install
```

Unattended installation:

```sh
robusta-magerun2.phar install [--magentoVersion[="..."]] [--magentoVersionByName[="..."]] [--installationFolder[="..."]] [--dbHost[="..."]] [--dbUser[="..."]] [--dbPass[="..."]] [--dbName[="..."]] [--installSampleData[="..."]] [--useDefaultConfigParams[="..."]] [--baseUrl[="..."]] [--replaceHtaccessFile[="..."]]
```

Example of an unattended Magento CE 2.0.0.0 dev beta 1 installation:

```sh
robusta-magerun2.phar install --dbHost="localhost" --dbUser="mydbuser" --dbPass="mysecret" --dbName="magentodb" --installSampleData=yes --useDefaultConfigParams=yes --magentoVersionByName="magento-ce-2.0.0.0-dev-beta1" --installationFolder="magento2" --baseUrl="http://magento2.localdomain/"
```
...
```sh
robusta-magerun2.phar sys:info
```
**Options:**
...
```sh
robusta-magerun2.phar sys:store:list [--format[="..."]]
```
...
```sh
robusta-magerun2.phar sys:website:list [--format[="..."]]
```
...
```sh
robusta-magerun2.phar config:env:create
```
...
```sh
robusta-magerun2.phar config:env:set <key> [<value>]
```
...
```sh
robusta-magerun2.phar config:env:set backend.frontName mybackend
robusta-magerun2.phar config:env:set crypt.key bb5b0075303a9bb8e3d210a971674367
robusta-magerun2.phar config:env:set session.redis.host 192.168.1.1
robusta-magerun2.phar config:env:set 'x-frame-options' '*'

robusta-magerun2.phar config:env:set --input-format=json queue.consumers_wait_for_messages 0
robusta-magerun2.phar config:env:set --input-format=json directories.document_root_is_pub true
robusta-magerun2.phar config:env:set --input-format=json cron_consumers_runner.consumers '["some.consumer", "some.other.consumer"]'
```
...
```sh
robusta-magerun2.phar config:env:delete <key>
```

Examples:

```sh
robusta-magerun2.phar config:env:delete system
robusta-magerun2.phar config:env:delete cache.frontend.default.backend
robusta-magerun2.phar config:env:delete cache.frontend.default.backend_options
```

### Show env.php settings

```sh
robusta-magerun2.phar config:env:show [options] [<key>]
```
...
```sh
robusta-magerun2.phar config:env:show  # whole content
robusta-magerun2.phar config:env:show backend.frontName
robusta-magerun2.phar config:env:show --format=json
robusta-magerun2.phar config:env:show --format=csv
robusta-magerun2.phar config:env:show --format=xml
```
...
```sh
robusta-magerun2.phar config:store:set [--scope[="..."]] [--scope-id[="..."]] [--encrypt] path value
```
...
```sh
robusta-magerun2.phar config:store:get [--scope="..."] [--scope-id="..."] [--decrypt] [--format[="..."]] [path]
```
...
```sh
robusta-magerun2.phar config:store:get web/* --magerun-script
```

### Delete Store Config

```sh
robusta-magerun2.phar config:store:delete [--scope[="..."]] [--scope-id[="..."]] [--all] path
```
...
```sh
robusta-magerun2.phar config:data:acl
```
...
```sh
robusta-magerun2.phar config:data:di <type>
```
...
```sh
robusta-magerun2.phar cache:list
```
...
```sh
robusta-magerun2.phar cache:clean [code]
```

If you would like to clean multiple cache types at once:

```sh
robusta-magerun2.phar cache:clean [code] [code] ...
```
...
```sh
robusta-magerun2.phar cache:flush [code]
```
...
```sh
robusta-magerun2.phar cache:list [--format[="..."]]
```

### Disable Magento cache

```sh
robusta-magerun2.phar cache:disable [code]
```
...
```sh
robusta-magerun2.phar cache:enable [code]
```
...
```sh
robusta-magerun2.phar admin:user:list [--format[="..."]]
```

### Change admin user password

```sh
robusta-magerun2.phar admin:user:change-password [username] [password]
```

### Delete admin user

```sh
robusta-magerun2.phar admin:user:delete [email|username] [-f]
```
...
```sh
robusta-magerun2.phar admin:token:create <username>
```
...
```sh
robusta-magerun2.phar db:query <sql-query>
```

**Example:**

```sh
robusta-magerun2.phar db:query "select * from store"
```

### Open MySQL Console

```sh
robusta-magerun2.phar db:console [options]
```
...
```sh
robusta-magerun2.phar db:dump
```

Only the mysqldump command:

```sh
robusta-magerun2.phar db:dump --only-command [filename]
```

Or directly to stdout:

```sh
robusta-magerun2.phar db:dump --stdout
```

Use compression (gzip cli tool has to be installed):

```sh
robusta-magerun2.phar db:dump --compression="gzip"
```
...
```sh
robusta-magerun2.phar db:dump --strip="@stripped"
```
...
```sh
robusta-magerun2.phar db:import
```
...
```sh
robusta-magerun2.phar db:add-default-authorization-entries
```
...
```sh
robusta-magerun2.phar dev:asset:clear [--theme="..."]
```
...
```sh
robusta-magerun2.phar dev:asset:clear
```

To clear assets for specific theme(s) only:

```sh
robusta-magerun2.phar dev:asset:clear --theme=Magento/luma
```

### List Themes

```sh
robusta-magerun2.phar dev:theme:list
```
...
```sh
robusta-magerun2.phar dev:module:create [-m|--minimal] [--add-blocks] [--add-helpers] [--add-models] [--add-setup] [--add-all] [-e|--enable] [--modman] [--add-readme] [--add-composer] [--add-strict-types] [--author-name [AUTHOR-NAME]] [--author-email [AUTHOR-EMAIL]] [--description [DESCRIPTION]] [-h|--help] [-q|--quiet] [-v|vv|vvv|--verbose] [-V|--version] [--ansi] [--no-ansi] [-n|--no-interaction] [--root-dir [ROOT-DIR]] [--skip-config] [--skip-root-check] [--skip-core-commands [SKIP-CORE-COMMANDS]] [--skip-magento-compatibility-check] [--] <command> <vendorNamespace> <moduleName>
```
...
```sh
robusta-magerun2.phar dev:module:detect-composer-dependencies [--only-missing] <directory>
```
...
```sh
robusta-magerun2.phar dev:translate:admin [--on] [--off]
```

Enable/disable inline translation feature for shop frontend:

```sh
robusta-magerun2.phar dev:translate:shop [--on] [--off] <store>
```

Set a translation (saved in translation table)

```sh
robusta-magerun2.phar dev:translate:set <string> <translate> [<store>]
```

Export inline translations

```sh
robusta-magerun2.phar dev:translate:export [--store=<storecode>] <locale> <filename>
```

### List modules

```sh
robusta-magerun2.phar dev:module:list [--vendor=VENDOR]
```
...
```sh
robusta-magerun2.phar dev:module:observer:list [--sort] <event> [<area>
```
...
```sh
robusta-magerun2.phar eav:attribute:view [--format[="..."]] entityType attributeCode
```
...
```sh
robusta-magerun2.phar giftcard:pool:generate
```

### Create a Gift Card

```sh
robusta-magerun2.phar giftcard:create [--website[="..."]] [--expires[="..."]] [amount]
```
...
```sh
robusta-magerun2.phar giftcard:info [--format[="..."]] [code]
```

### Remove a Gift Card

```sh
robusta-magerun2.phar giftcard:remove [code]
```
...
```sh
robusta-magerun2.phar sys:setup:compare-versions [--ignore-data] [--log-junit="..."] [--format[="..."]]
```
...
```sh
robusta-magerun2.phar sys:setup:change-version module version
```
...
```sh
robusta-magerun2.phar sys:setup:downgrade-versions
```
...
```sh
robusta-magerun2.phar media:dump [--strip] [filename]
```
---
...
```sh
robusta-magerun2.phar integration:list
```

#### Create a new integration

```sh
robusta-magerun2.phar integration:create [options] [--] <name> <email> <endpoint>
```
...
```sh
robusta-magerun2.phar integration:create "My new integration 10" foo@example.com https://example.com -r Magento_Catalog::catalog_inventory -r Magento_Backend::system_other_settings
```
...
```sh
robusta-magerun2.phar integration:show --format[=FORMAT] <name_or_id> [key]
```

Example (print only Access Key):

```sh
robusta-magerun2.phar integration:show 1 "Access Key"
```

#### Delete integration

```sh
robusta-magerun2.phar integration:delete <name_or_id>
```
...
```sh
robusta-magerun2.phar dev:console [--area=AREA] <arg>
```
...
Example:
    ...
    robusta-magerun2 dev:console --area=adminhtml
...
```sh
robusta-magerun2.phar dev:console "$a = 1; call cache:flush; ls;"
```
...
### robusta-magerun Shell

If you need autocompletion for all robusta-magerun commands you can start
with `shell command`.

```sh
robusta-magerun2.phar shell
```

### robusta-magerun Script

Run multiple commands from a script file.

```sh
robusta-magerun2.phar script [-d|--define[="..."]] [--stop-on-error] [filename]
```
...
```sh
echo "cache:flush" | robusta-magerun2.phar script
```

```sh
robusta-magerun2.phar script < filename
```
...
```sh
#!/usr/bin/env robusta-magerun2.phar script
...
# Use resolved variables from robusta-magerun in shell commands
...
```sh
robusta-magerun2.phar script -d foo=bar filename
...
```sh
robusta-magerun2.phar cms:block:toggle [blockId]
```
...
```sh
robusta-magerun2.phar admin:user:change-status [user] [--activate] [--deactivate]
```
...
Simply place a script in folder */usr/local/share/robusta-magerun2/scripts* or in your home dir
in folder *<HOME>/.robusta-magerun2/scripts*.
...
```sh
$ robusta-magerun2.phar script:repo:list [--format[="..."]]
```

If you want to execute a script from the repository this can be done by *script:repo:run* command.

```sh
$ robusta-magerun2.phar script:repo:run [-d|--define[="..."]] [--stop-on-error] [script]
```
...
https://github.com/netz98/robusta-magerun2/wiki
