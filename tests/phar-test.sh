...
function test_magerun_commands() {
	...
	# $> ./robusta-magerun2.phar --skip-core-commands list --format json | jq -r '.commands[] | .name'
	...
	#  config:store:set
	assert_command_contains "config:store:set robusta/magerun/example 1" "robusta/magerun/example => 1"
	#  config:store:get
	assert_command_contains "config:store:get robusta/magerun/example" "robusta/magerun/example"
	#  config:store:delete
	assert_command_contains "config:store:delete robusta/magerun/example" "deleted path"
	#  customer:create
	...
	cleanup_files_in_magento "app/code/Robusta/Magerun123"
...
function test_magento_core_commands() {
	...
	#  help (covered by robusta-magerun2)
	...
	#  list (covered by robusta-magerun list command)
...
function test_custom_module() {
	mkdir -p $MAGENTO_ROOT_DIR/lib/robusta-magerun2/modules;
	cp -r tests/example-module $MAGENTO_ROOT_DIR/lib/robusta-magerun2/modules/example-module;

	assert_command_contains "magerun:example-module:test" "98.00"

	rm -Rf $MAGENTO_ROOT_DIR/lib/robusta-magerun2/modules/example-module
}
