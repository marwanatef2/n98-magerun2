...
use Exception;
use Robusta\Util\Markdown\VersionFilePrinter;
use Phar;
use PharException;
...
{
    public const VERSION_TXT_URL_UNSTABLE = 'https://raw.githubusercontent.com/netz98/robusta-magerun2/develop/version.txt';
    public const MAGERUN_DOWNLOAD_URL_UNSTABLE = 'https://files.magerun.net/robusta-magerun2-dev.phar';
    public const VERSION_TXT_URL_STABLE = 'https://raw.githubusercontent.com/netz98/robusta-magerun2/master/version.txt';
    public const MAGERUN_DOWNLOAD_URL_STABLE = 'https://files.magerun.net/robusta-magerun2.phar';
    public const CHANGELOG_DOWNLOAD_URL_UNSTABLE = 'https://raw.github.com/netz98/robusta-magerun2/develop/CHANGELOG.md';
    public const CHANGELOG_DOWNLOAD_URL_STABLE = 'https://raw.github.com/netz98/robusta-magerun2/master/CHANGELOG.md';
    ...
    protected function configure()
    {
        $this
            ...
            ->setDescription('Updates robusta-magerun2.phar to the latest version.')
            ->setHelp(
                <<<EOT
The <info>self-update</info> command checks github for newer
versions of robusta-magerun2 and if found, installs the latest.

<info>php robusta-magerun2.phar self-update</info>
...
EOT
    ...
    protected function execute(InputInterface $input, OutputInterface $output)
    {
        ...
        if (!is_writable($tempDirectory = dirname($tempFilename))) {
            throw new RuntimeException(
                'robusta-magerun2 update failed: the "' . $tempDirectory .
        ...
        if (!is_writable($localFilename)) {
            throw new RuntimeException(
                'robusta-magerun2 update failed: the "' . $localFilename . '" file could not be written'
            );
        }
        ...
        if ($this->isOutdatedVersion($latestVersion, $loadUnstable)) {
            ...
            try {
                ...
                $output->writeln('<info>Successfully updated robusta-magerun2</info>');
                $output->writeln('<info>---------------------------------</info>');
            ...
            $output->writeln("<info>You are using the latest robusta-magerun2 version.</info>");
        }
    }
    ...
    private function downloadNewPhar(OutputInterface $output, string $remoteUrl, string $tempFilename)
    {
        ...
        if (!file_exists($tempFilename)) {
            $output->writeln('<error>The download of the new robusta-magerun2 version failed for an unexpected reason');
        }
    }
